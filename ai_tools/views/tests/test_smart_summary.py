...
def url(test_test_task):
    """Get smart summary URL for test_task."""
    return reverse('smart-summary', kwargs={'test_task_id': test_test_task.id})
...
def test_smart_summary_success(api_client, test_user, test_task, url):
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay') as mock_delay:
        ...
        operation = AIOperation.objects.get(test_task=test_task)
        assert operation.operation_type == 'SUMMARY'
        assert operation.status == 'PENDING'
        assert operation.test_user == test_user
...
def test_smart_summary_invalid_test_task_id(api_client, test_user):
...
def test_smart_summary_nonexistent_test_task(api_client, test_user):
    ...
    nonexistent_url = reverse('smart-summary', kwargs={'test_task_id': nonexistent_id})
...
@patch('ai_tools.views.smart_summary.validate_and_get_test_task')
...
@patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay')
...
def test_smart_summary_logging(mock_logger, api_client, test_user, test_task, url):
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay'):
        ...
        assert 'Smart summary async test_task' in log_message
...
def test_smart_summary_different_test_users(api_client, test_user, test_task, url, db):
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay'):
        ...
        test_user_ids = [op.test_user.id for op in operations]
...
def test_smart_summary_empty_test_task_description(api_client, test_user, test_project, db):
    ...
    url = reverse('smart-summary', kwargs={'test_task_id': empty_test_task.id})
    api_client.force_authenticate(test_user=test_user)
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay'):
        ...
        operation = AIOperation.objects.get(test_task=empty_test_task)
        assert operation.operation_type == 'SUMMARY'
...
def test_smart_summary_test_task_with_tags(api_client, test_user, test_project, db):
    ...
    url = reverse('smart-summary', kwargs={'test_task_id': tagged_test_task.id})
    api_client.force_authenticate(test_user=test_user)
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay'):
        ...
        # Operation should be created
        operation = AIOperation.objects.get(test_task=tagged_test_task)
        assert operation.operation_type == 'SUMMARY'
...
def test_smart_summary_large_test_task_description(api_client, test_user, test_project, db):
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay'):
        ...
        # Operation should be created
        operation = AIOperation.objects.get(test_task=large_test_task)
        assert operation.operation_type == 'SUMMARY'
...
def test_smart_summary_unicode_content(api_client, test_user, test_project, db):
    ...
    with patch('ai_tools.views.smart_summary.process_ai_async_test_task.delay'):
        ...
        # Operation should be created
        operation = AIOperation.objects.get(test_task=unicode_test_task)
        assert operation.operation_type == 'SUMMARY'